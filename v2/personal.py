...
from utils.replays_getter import select_replay
...
def main():
    ...
    replays = select_replay()
    skin = "vv_idke_trail"
    for replay in replays:
        replay_name = replay.rsplit("/", 1)[1].replace(".osr", "")
        ...
        if program_args.record:
            record(config, replay, replay_name, skin)
        ...
        with open(replay, "rb") as file:
            ...
            video_title =  f"{parsed_replay.username} {map_data.song_title} [{map_data.difficulty}]".replace("\"", "").replace("'", "").replace("/", "")
            ...
            if program_args.premiere:
                default_edit(os.path.join(config["danser_path"], "videos", f"{replay_name}.mp4").replace("/", "\\"), video_title)
