...
library = ctypes.cdll.LoadLibrary(os.path.join(
    str(os.path.abspath(__file__)).rsplit(os.sep, 1)[0], "main.so"))


def get_star_rating(difficulty_path="", mods="NoMod"):
    """
    Get star rating from specific beatmap!
    Usage:
    - difficulty_path: Type path to map's .osu difficulty file.
    ...
    if difficulty_path == "":
        return None, "No difficulty_path set"
    ...
    if mods == None:
        mods = ""
    mods.replace("Flashlight", "FlashLight")
    ...
    # Run commands
    logger.debug(
        f"Getting star rating from {difficulty_path} with mods {mods}")
    result = go_get_star(difficulty_path.encode("utf-8"), mods.encode("utf-8"))
...
def get_pp(difficulty_path="", mods="NoMod", max_combo="-1", n300s="-1", n100s="0", n50s="", nmisses=""):
    """
    ...
    # Run commands
    result = go_get_pp(difficulty_path.encode("utf-8"), mods.encode("utf-8"), max_combo.encode("utf-8"),
                       n300s.encode("utf-8"), n100s.encode("utf-8"), n50s.encode("utf-8"), nmisses.encode("utf-8"))
...
def run():
    ...
    b = get_star_rating(difficulty_path=path, mods=mods)
    print(b, type(b))
