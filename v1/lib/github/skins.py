...
class SkinHandler:
    ...
    def get_skin_by_name(self, target_skin: str) -> None:
        ...
        elif target_skin == "Skin Privada ( me mande no privado luis10barbo#3251 )":
            return {"link": "Skin Privada :("}
        logger.error(
            f"No skin matching to '{target_skin}' found at local skins, making github request")

        self.get_skins_from_github()
        if target_skin in self.skins:
            return self.skins[target_skin]
        logger.error(
            f"No skin matching to '{target_skin}' found at github skins", True)
